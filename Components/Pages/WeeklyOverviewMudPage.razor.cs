...
public partial class WeeklyOverviewMudPage : ComponentBase
{
    [Inject]
    ...
    protected Task HandleDateChangedAsync(DateTime? date)
    {
        if (date.HasValue)
        {
            NavigateToWeek(date.Value);
        }
    ...
    {
        Navigation.NavigateTo($"/weekly-reports/mud?date={date:yyyy-MM-dd}");
    }
}
